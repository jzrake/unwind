...
void dXdt(double *X, double *dXdt)
{
  ...
  dXdt[3] = a/(sqrt(3.0)*M4) * sqrt( V + sqrt(g*g -1)/(g*chi) * S1(z)/pow(a,3) + 2.0*sig*g );  // = a'

  double term1 = -3.0 * dXdt[3]/a * (g*g - 1)/g;
  double term2 = -0.5 * dVdz * sqrt(g*g - 1) / (g * sig);
  double term3 = -0.5 * (g*g - 1)/(g*chi) * 1/( sig * pow(a,3) ) * S2(z);
  double denom = 1.0 + 0.5/(g*chi) * ( 1/( g*sqrt(g*g-1) ) - 1/chi ) * S1(z)/( sig * pow(a,3) );
...
void load_polylog_table()
{
  ...
  //  printf("ImpactParameter = %f\n", ImpactParameter);
  //  printf("DimensionOfBrane = %d\n", DimensionOfBrane);
  //  printf("Size of polylog table = %d\n", PolylogTableN);
...
{
  int i;
  double *S1_integrand = (double*) malloc(IterationNumber * sizeof(double));
  double *S1_time      = (double*) malloc(IterationNumber * sizeof(double));

  //  printf("IterationNumber=%d\n", IterationNumber);
  ...
  for (i=0; i<IterationNumber; ++i) {
    ...
    S1_integrand[i] = dp * pow(ms,p+2)/pow(2*PI, p) * pow(chi,0.5*p)* F_function(chi) * abs(z-zi) * pow(ai,3) * K;
    S1_time[i]      = ti;
  }
  double definite_integral =
    numerically_integrate(S1_time, S1_integrand, IterationNumber);
  free(S1_integrand);
  free(S1_time);
...
{
  int i;
  double *S2_integrand = (double*) malloc(IterationNumber * sizeof(double));
  double *S2_time      = (double*) malloc(IterationNumber * sizeof(double));
  ...
  for (i=0; i<IterationNumber; ++i) {
    ...
    S2_integrand[i] = dp * pow(ms,p+2)/pow(2*PI, p) * pow(chi,0.5*p)* F_function(chi) * abs(z-zi)/(z-zi) * pow(ai,3) * K;
    S2_time[i]      = ti;
  }
  double definite_integral =
    numerically_integrate(S2_time, S2_integrand, IterationNumber);
  free(S2_integrand);
  free(S2_time);
  return definite_integral;
